...
#define RATIO_SCALE_LUT				3u

#define SCALE_LUT(arg)\
	(arg / RATIO_SCALE_LUT)
...
void APP_MOT_CTRL_Task(void)
{
  static uint8_t rampPresc = 0u;
    ...
    if (10u == (LL_TIMERS_TIMEDIFF(LL_ISR_SysTicks, CurrTicks)))
    {
    	...
    	Svm.Angle += SpeedRamp.CurrVal;
    	Svm.Amp = AmplRamp.CurrVal;
    	printf("Angle:%d Amp:%d          ", Svm.Angle, Svm.Amp);
    	GenSvm(&Svm);
    }
...
static void GenSvm(Svm_t *pSvm)
{
  ...
  uint16_t idx = 0u;
	  	...
	  	idx = (Offset >> 7u);
		phADutyCycle = SCALE_LUT(SvmLut[idx]);
		Offset = (Svm.Angle + PhAngleOffset[IDX_PHASE_B]);
		idx = (Offset >> 7u);
		phBDutyCycle = SCALE_LUT(SvmLut[idx]);
		Offset = (Svm.Angle + PhAngleOffset[IDX_PHASE_C]);
		idx = (Offset >> 7u);
		phCDutyCycle = SCALE_LUT(SvmLut[idx]);

		phADutyCycle = ((phADutyCycle * Svm.Amp) / 100u);
		phBDutyCycle = ((phBDutyCycle * Svm.Amp) / 100u);
		phCDutyCycle = ((phCDutyCycle * Svm.Amp) / 100u);

	    LL_TIMERS_SetDutyPhA(phADutyCycle);
	    LL_TIMERS_SetDutyPhB(phBDutyCycle);
	    LL_TIMERS_SetDutyPhC(phCDutyCycle);

    printf("phase A: %d B: %d C: %d\n", phADutyCycle, phBDutyCycle, phCDutyCycle);
}
